...
export function ProjectCard({
  ...
  return (
    <div className="group relative border border-zinc-800 rounded-md p-6 hover:border-cyan-700 hover:bg-zinc-900/40 transition-all">
      {/* Stretched main Link || vangt clicks op de hele card (Z-index zorgt dat 'm onder de inhoud zit voor selectie/visuals; pointer-events-none op content laat clicks doorvallen naar de Link) */}
      <Link
        href={href}
        className="absolute inset-0 rounded-md"
        aria-label={`Open case study voor ${title}`}
      >
          ...
          <div className="flex items-center gap-3 shrink-0 ml-auto">
            {externalUrl && (
              <a
                ...
                className="pointer-events-auto relative z-10 text-[10px] font-mono uppercase tracking-widest text-cyan-400 hover:text-cyan-300 transition-colors"
              >
